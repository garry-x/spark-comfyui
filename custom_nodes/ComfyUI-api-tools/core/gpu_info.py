...
class CGPUInfo:
    ...
    def __init__(self):
        try:
            ...
            logging.info('Pynvml (Nvidia) initialized.')
        except Exception as e:
            logging.error('Could not init pynvml (Nvidia).' + str(e))

        # if not self.pynvmlLoaded:
        #     try:
        #         rocml.smi_initialize()
        #         self.pyamdLoaded = True
        #         print('Pyrsmi (AMD) initialized.')
        #     except Exception as e:
        #         print('Could not init pyrsmi (AMD).' + str(e))
        ...
        try:
            ...
            logging.error('Could not pick default device.' + str(e))
        ...
        if 'zluda' in self.torchDevice or 'ZLUDA' in self.torchDevice or 'Zluda' in self.torchDevice:
            logging.warn('ZLUDA detected. GPU monitoring will be disabled.')
        ...
        if self.anygpuLoaded and self.deviceGetCount() > 0:
            ...
            for deviceIndex in range(self.cudaDevicesFound):
                ...
                logging.info(f"{deviceIndex}) {gpuName}")
            ...
            self.cuda = True
            logging.info(self.systemGetDriverVersion())
        else:
            logging.warn('No GPU with CUDA detected.')
        ...
        if self.cuda and self.cudaAvailable and self.torchDevice == 'cpu':
            logging.warn('CUDA is available, but torch is using CPU.')

    def getInfo(self):
        logging.debug('Getting GPUs info...')
        return self.gpus

    def getStatus(self):
        # logging.debug('CGPUInfo getStatus')
        gpuUtilization = -1
        gpuTemperature = -1
        vramUsed = -1
        vramTotal = -1
                ...
                for deviceIndex in range(self.cudaDevicesFound):
                    ...
                    if self.switchGPU and self.gpusUtilization[deviceIndex]:
                        try:
                            ...
                            if str(e) == "Unknown Error":
                                logging.error('For some reason, pynvml is not working in a laptop with only battery, try to connect and turn on the monitor')
                            else:
                                logging.error('Could not get GPU utilization.' + str(e))

                            logging.info('Monitor of GPU is turning off (not on UI!)')
                    ...
                    if self.switchTemperature and self.gpusTemperature[deviceIndex]:
                        try:
                            ...
                            logging.error('Could not get GPU temperature. Turning off this feature. ' + str(e))
